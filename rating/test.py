...
class CourseProgressTests(MainSetup):
    ...
    def test_update_course_progress(self):
        ...
        # Создаём курс и группу
        course = baker.make(Course, author=teacher)
        course_group = baker.make(CourseGroup, course=course)
        CourseGroupAssignTeacher(group=course_group, user=teacher).save()

        # Создаём урок и прогресс для студента
        lesson = baker.make(Lesson, course=course)
        instance = baker.make(CourseProgress, user=students[0], course=course)
...
class LessonProgressTests(MainSetup):
    ...
    def test_create_lesson_progress(self):
        def check(check_status):
            self.assertEqual(response.status_code, check_status)
            if check_status == status.HTTP_201_CREATED:
                self.assertEqual(LessonProgress.objects.count(), amount + 1)
            else:
                self.assertEqual(LessonProgress.objects.count(), amount)
        ...
        course = baker.make(Course, author=self.user)
        lesson = baker.make(Lesson, course=course)
        group = baker.make(CourseGroup, course=course)
    ...
    def test_delete_lesson_progress(self):
        ...
        (course_group := baker.make(CourseGroup, course=course)).save()
        CourseGroupAssignTeacher(group = course_group, user=teacher).save()
        CourseGroupAssignStudent(group = course_group, user=students[1]).save()
        url = reverse('lessonprogress-detail', kwargs={'pk': instance.id})
        response = self.client.delete(url, data, format='json')
        self.assertEqual(response.status_code, status.HTTP_204_NO_CONTENT)
        self.assertEqual(Problem.objects.count(), amount - 1)
